...
The entirety of the project, including it's design, planning and implementation is maintained by the author, @mariolfh on Github \cite{githubinc.MariolfhOverview2025}. As a computer science graduate, getting into contact with all sort of new concepts and ideas is something marvelous. So, the hopes of having several important and key tools in one place and at hand, mixed with a love for terminals, made the original thought that would soon evolve into this project. 

\section{What's \emph{mars}?}
Mars is a command-line interface tool designed to assist developers and power users with daily tasks right from the terminal. It serves as a personal tool-set that runs entirely offline, focusing on simplicity.
...
\section{Scope}
This program means to bring several most common functionalities into the command line interface of users who prefer it's use over graphical user interfaces. Meant to assist in a simple way while staying local and offline, \emph{mars} stands as the base for more programs of it's type.
...
This piece of software began from a crave the author had during college. On the average developer's day to day work flow filled with tools and graphical user interfaces, getting focus can be difficult, and having the right tools for the job can very often be a dread. With that in mind, the author intends to present a simple, offline, always accessible, terminal based and tool rich program that can satisfy power users' daily needs.

\section{Product Functions}
With the goal to create something power users enjoy, and thinking on designing functionalities that can be used on a daily basis locally, offline and with simplicity, the functions this program posess are:
\begin{itemize}
...
This is the representation of the file structure used in the project. It contains the folders and files that are being published on GitHub and are related to the correct functioning of the application. It contains two folders, one containing all documentation related files and the other the code files.

\dirtree{%
.1  documentation.
.2 Project Documentation.bib.
.2 Project Documentation.pdf.
.2 Project Documentation.tex.
.1 src.
.2 clipboard.rs.
.2 fileops.rs.
.2 main.rs.
...
\begin{itemize}
\item{`Project Documentation.bib'}: This file contains the bibliographical information used in the documentation. All the sources I stored on Zotero are automatically exported to this file to then be cited in this document.
\item{`Project Documentation.pdf'}: Final document file for the project, rendered in a standard digital document format.
\item{`Project Documentation.tex'}: Raw code for the document to be rendered of. It included \TeX \space and \LaTeX \space code that generates the document.
\item{`clipboard.rs'}: File storing the code that manages the clipboard operations, which are copy and paste.
\item{`fileops.rs'}: File storing the code that takes care of the file operations, such as getting the file, setting values in the file and checking if the file exists.
\item{`main.rs'}: Main file of the project which manages the code that understands the commands given and what to do with the arguments provided.
\item{`Cargo.toml'}: Information file that stores the libraries that the project uses along with it's versions.
\end{itemize}

\section{Commands}
This section goes deep into the usage of \emph{mars} \cite{fernandezMariolfhMars2025}, explaining the main functionalities along with code snippets as demonstration.
An important thing to notice is that \emph{mars} works with the following structure:
\begin{lstlisting}
// Basic command call
$ mars <command>
\end{lstlisting}
In some specific cases, the command will require one or more arguments to work properly. These arguments are required for operation and will be specified as needed in this document.
...
\subsection{String}
The command string works with two arguments: one indicating the operation to be performed and the other to specify the value that is used in said operation. Commands like this that use a value can work with the * placeholder, making the program use the latest stored value for the task instead of a supplied value.
\begin{lstlisting}
// Basic command call
$ mars string <operation> <value>
$ mars string <operation> *
\end{lstlisting}
...
\begin{lstlisting}
// Basic command call
$ mars string lowercase <STRING>
<string>
\end{lstlisting}

\subsubsection{Size}
This function requires a string as a parameter. The string's length in characters is then calculated and returned.
\begin{lstlisting}
// Basic command call
$ mars string sizee <STRING>
8
\end{lstlisting}


\subsection{Copy}
This function doesn't require any parameters to work. It takes the latest value that was processed with Mars and inserts it on the system's clipboard to be used across the system.
\begin{lstlisting}
// Basic command call
$ mars copy
Last used element copied from Mars to the clipboard.
\end{lstlisting}

\subsection{Paste}
This function doesn't require any parameters to work. It takes whatever the system's clipboard has in store and makes it available inside Mars for later use through the * placeholder.
\begin{lstlisting}
// Basic command call
$ mars paste
Clipboard contents pasted into Mars from the clipboard.
\end{lstlisting}
